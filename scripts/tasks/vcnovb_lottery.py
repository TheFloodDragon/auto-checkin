#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""站点模板：VC API（sub.vcnovb.cn）幸运轮盘每日抽取。

轮盘抽取本身就是本站的每日任务：优先纯 HTTP 调用轮盘接口；token 不可用时由登录
候选链依次尝试 refresh_token、纯 HTTP 账密登录，最后才启动浏览器。浏览器兜底也只
调用站点真实轮盘 API，不依赖动画按钮。

这是「广义每日任务」的典型样本：没有额度、没有签到接口，结果是「中了什么奖」。
自定义文本列因此展示奖品名而不是金额。
"""
...
API_PREFIX = "/api/v1"
SUMMARY_ROUTE = f"{API_PREFIX}/lottery?{TIMEZONE_QUERY}"
HISTORY_ROUTE = f"{API_PREFIX}/lottery/history?page=1&page_size=20&{TIMEZONE_QUERY}"
DRAW_ROUTE = f"{API_PREFIX}/lottery/pools/{POOL_KEY}/draw?{TIMEZONE_QUERY}"
...
MANIFEST = TemplateManifest(
    id="vcnovb_lottery",
    title="VC API 幸运轮盘",
    description="每日抽取一次幸运轮盘（无额度概念，结果是奖品）",
    login=(
        ...
        LoginOption(
            ...
            args=ArgSchema(
                (
                    ArgSpec("email", env="VCNOVB_EMAIL", secret=True, title="邮箱"),
                    ArgSpec("password", env="VCNOVB_PASSWORD", secret=True, title="密码"),
                )
            ),
        ),
    ...
    task=(
        TaskOption(
            ...
            priority=10,
            title="幸运轮盘",
            # 本站用自定义 lottery 接口而非标准签到接口：探测 /api/v1/check-in/status
            # 只会产生无用的 404 日志，结果确认也由本模板自己完成。
            owns=frozenset({"detect", "confirm"}),
            args=ArgSchema((ArgSpec("start_path", default="/lottery", title="入口路径"),)),
        ),
    ),
    display=DisplayDefaults(text_label="奖品"),
    endpoints={
        "prefix": API_PREFIX,
...
def _outcome(
    ...
    *,
    ...
    「没中奖」也是**成功**：任务确实完成了，只是奖品为空。把它判成失败会让用户每天
    ...
    message = str(data.get("result_message") or "抽奖完成")
    factory = already_done if already else success
    text = _result_text(record) if record else ""
    extras: tuple[tuple[str, str], ...] = ()
    remaining = _remaining(state)
    if remaining:
        extras = (("剩余次数", str(remaining)),)
    return factory(message, data=data).with_display(
        DisplaySpec(text=text, text_label="奖品" if text else "", extras=extras)
    )


def _is_no_chance(exc: TaskError) -> bool:
...
def _idempotency_key(ctx: Any, state: dict[str, Any]) -> str:
    """每个账号/周期固定一个键；只传摘要，不泄露账号标识。

    并发或重试时靠它让服务端把两次 POST 认作同一次抽取——没有它，一次网络重传就
    可能消耗掉第二天的机会。
    """
    seed = "|".join(
        (
            str(ctx.account.base_url),
            str(ctx.account.id),
            POOL_KEY,
...
def _already_outcome(ctx: Any, state: dict[str, Any]) -> Outcome:
    history = ctx.http.get(HISTORY_ROUTE)
    record = _matching_history(history, state.get("period_key"))
    outcome = _outcome(state, record, already=True, raw=history)
...
async def http_attempt(ctx: Any) -> Outcome:
    ...
    if not bool(state.get("active")) or not bool(pool.get("enabled", True)):
        # 活动没开门不是故障：重试与开浏览器都不会改变结果。
        raise NotApplicable("普通抽奖当前未开放", payload=summary)

    if _remaining(state) <= 0:
        return _already_outcome(ctx, state)

    ctx.log("今日轮盘仍有次数，调用抽取接口")
    try:
        draw = ctx.http.request(
            "POST",
            DRAW_ROUTE,
            json_body={},
            headers={"Idempotency-Key": _idempotency_key(ctx, state)},
        )
    except TaskError as exc:
...
async def _browser_draw(ctx: Any, lease: Any, page: Any) -> Outcome:
    """浏览器兜底：完成真实登录后仍直接调用轮盘 API，不依赖动画按钮。"""
    helpers = PageHelpers(ctx, lease, page)
    context = lease.context
    opts = common.parse_options(SPEC, ctx.args)
    ...
    if not await common.authenticated(page, origin):
        ...
        if not await common.authenticated(page, origin):
            return need_login(
                ...
                data={"target_url": resolved_url, **login_detail},
            )

    lease.mark_authenticated()
    ctx.log("浏览器登录态已验证，直接调用幸运轮盘 API")
    try:
    ...
    if not bool(result.get("ok")):
        ...
        if status in {401, 403}:
            return need_login(
                "VC API 幸运轮盘登录态已失效",
                data={"response_status": status, "reason": reason, **login_detail},
            )
        return helpers.error(
    ...
    return _outcome(
        ...
        raw=result.get("raw"),
        extra={"response_status": status, "source": "browser_api", **login_detail},
    )
