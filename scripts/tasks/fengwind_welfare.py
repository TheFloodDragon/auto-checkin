#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""站点模板：Fengwind API 福利站每日签到。

该站不是标准 New API OAuth：登录需要 LinuxDO → Fengwind 主站 → 福利站的**双层
...
MANIFEST = TemplateManifest(
    ...
    task=(
        TaskOption(
            ...
            priority=10,
            title="福利站签到",
            # 状态、签到、历史与结果确认全部由本模板调用站点原生 /api 完成；
            # 通用层不得先用别的 profile 猜 /api/v1/* 端点。
...
def _request(ctx: Any, method: str, path: str, body: dict[str, Any] | None = None, *, token: str = "") -> Any:
    ...
    ``token`` 非空时覆盖 ctx 已注入的认证——浏览器路径拿到的是页面里的
    ``welfare_token``，与登录阶段注入的可能不是同一枚。
    """
    headers = {"Accept": "application/json"}
    value = normalize_access_token(token)
    if value:
        headers["Authorization"] = f"Bearer {value}"
    elif not ctx.http.headers.get("Authorization"):
        raise TaskError(f"{SITE_LABEL}缺少 welfare_token", status=401, reason="need_login",
                        payload={"reason": "WELFARE_TOKEN_MISSING"})
    payload = ctx.http.request(
        method,
        f"{API_PREFIX}{path}",
        json_body=(body or {}) if method.upper() in {"POST", "PUT", "PATCH"} else None,
        headers=headers,
    )
    # 站点用 {code,...} 信封表达业务错误，HTTP 状态仍是 200。
    if isinstance(payload, dict) and payload.get("code") not in (None, 0, "0"):
        raise TaskError(extract_message(payload), payload=payload)
    return payload


def _optional_request(ctx: Any, method: str, path: str, *, token: str = "") -> Any:
    try:
        return _request(ctx, method, path, token=token)
    except TaskError:
        return None


def _already_outcome(
    status: dict[str, Any],
    level: dict[str, Any],
    history: Any,
) -> Outcome:
    today = _today(status)
    return already_done(
        _message(already=True, today=today),
        data={
            "completion_signal": "welfare_status",
            "welfare_status": _safe_status_detail(status, level, history),
        },
    ).with_display(_display(today))


def _display(today: dict[str, Any], *, awarded: Any = None) -> DisplaySpec:
    """自定义文本列展示积分；本站没有「额度」概念，所以列头是「积分」。"""
    amount = _amount(today)
    text = _credit_label(amount) if amount is not None else ""
    extras: tuple[tuple[str, str], ...] = ()
    gained = _number(awarded)
    if gained is not None and gained != 0:
        extras = (("获得", _credit_label(gained)),)
    return DisplaySpec(text=text, text_label="积分" if text else "", extras=extras)


def _is_already_error(exc: TaskError) -> bool:
...
def http_checkin(ctx: Any, *, token: str = "") -> Outcome:
    """执行福利站每日签到并返回可展示的结论。

    ``token`` 由浏览器路径传入（页面里的 welfare_token）；纯 HTTP 路径留空，用登录
    阶段注入的凭据。
    """
    ctx.log("读取 Fengwind 福利站签到状态")
    before_raw = _request(ctx, "GET", STATUS_PATH, token=token)
    before = _status_data(before_raw)
    level = _status_data(_optional_request(ctx, "GET", "/level", token=token))
    history_raw = _optional_request(ctx, "GET", HISTORY_PATH, token=token)

    if before.get("checked_in_today") is True:
        outcome = _already_outcome(before, level, history_raw)
        ctx.log(outcome.message)
        return outcome
    if before.get("enabled") is False:
        # 站点没开门：不是故障，重试与开浏览器都不会改变结���。
        raise NotApplicable(f"{SITE_LABEL}签到功能暂未开放", payload=before_raw)
    if level.get("checkin_eligible") is False:
        # 等级门禁是**账号状态**，与站点是否开放不同：需要用户去提升等级，
        # 因此归为需配置而不是未开放（旧实现在这里报 403 失败，噪声很大）。
        qualification = level.get("checkin_qualification") or "资格不足"
        raise ConfigError(
            f"{SITE_LABEL}当前不具备签到资格（{qualification}）；这是账号等级门禁，"
            "达标后会自动恢复，可先为该账号打开 policy.tolerate_failure。",
            payload=level,
        )

    ctx.log("今日尚未签到，调用 Fengwind 福利站签到接口")
    try:
        action_raw = _request(ctx, "POST", CHECKIN_PATH, {}, token=token)
    except TaskError as exc:
        # POST 发生网络/服务端不确定错误时，先读状态确认服务端是否已经记账。
        after_raw = _optional_request(ctx, "GET", STATUS_PATH, token=token)
        after = _status_data(after_raw)
        if after.get("checked_in_today") is True or _is_already_error(exc):
            outcome = _already_outcome(after or before, level, history_raw)
            ctx.log(outcome.message)
            return outcome
        raise

    action = _as_dict(unwrap_data(action_raw))
    after_raw = _request(ctx, "GET", STATUS_PATH, token=token)
    after = _status_data(after_raw)
    history_after = _optional_request(ctx, "GET", HISTORY_PATH, token=token)
    ...
    if not confirmed:
        raise TransientError(
            ...
            payload={"status": after, "action": action},
        )
    ...
    message = _message(already=False, today=today, fallback=action)
    ctx.log(message)
    return success(message, data=detail).with_display(_display(today, awarded=amount))
...
async def _browser_checkin(ctx: Any, lease: Any, page: Any) -> Outcome:
    ...
    if not verified:
        ctx.log("Fengwind welfare_token 不可用，开始双层 LinuxDO SSO")
    ...
    ctx.log("Fengwind 福利站登录态验证成功，执行签到 API")
    try:
        result = http_checkin(ctx, token=token)
    except TaskError as exc:
        status = int(exc.status or 0)
        data = {"auth_verified": True, "response_status": status, "target_url": origin}
        if status in {401, 403} and "资格" not in str(exc.message):
            return need_login(exc.message, data=data)
        return exc.to_outcome().with_data(**data)
    return result.with_data(auth_verified=True, oauth_provider="linuxdo", source="browser_api")
