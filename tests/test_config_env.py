...
config.settings 在导入时一次性读取环境变量，因此每个用例用 importlib.reload 重新加载。
"""
...
def test_file_lock_uses_config_timeout(monkeypatch, tmp_path) -> None:
    """文件锁超时必须取自 config，而非硬编码 30.0。"""
    from config import paths as accounts_store

    # 签名默认值为 None，运行时才解析为 FileLockConfig.DEFAULT_TIMEOUT，
    # 这样改 config 才真的生效（旧实现把 30.0 写死在签名里）。
    import inspect

    sig = inspect.signature(accounts_store.file_lock.__wrapped__)
