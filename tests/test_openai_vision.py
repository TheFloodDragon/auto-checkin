...
from core.errors import TaskError as ApiError
...
def test_response_format_400_retries_without_response_format() -> None:
    ...
    def transport(_url: str, **kwargs):
        ...
        if len(requests) == 1:
            raise ApiError("bad request secret-key", status=400, payload={"error": "unsupported response_format"})
    ...
    def make(sequence):
        ...
        def transport(_url: str, **_kwargs):
            ...
            if isinstance(item, int):
                raise ApiError("x", status=item, payload={"error": {"message": "upstream"}})
...
def test_statusless_transient_network_error_retries_but_schema_error_does_not() -> None:
    ...
    status=None、按瞬时错误上报；旧逻辑只看状态码，单次即放弃。
    """
...
def test_non_400_error_does_not_retry_or_leak_key() -> None:
    ...
    def transport(_url: str, **_kwargs):
        nonlocal calls
        calls += 1
        raise ApiError("secret-key rejected", status=401, payload={"message": "secret-key invalid"})
...
def test_second_failure_after_400_does_not_leak_key() -> None:
    ...
    def transport(_url: str, **_kwargs):
        ...
        calls += 1
        if calls == 1:
            raise ApiError("unsupported", status=400, payload=None)
