...
def test_refresh_token_reaches_task_params() -> None:
    ...
    assert core.task_params(row, {})["credentials"]["refresh_token"] == "rt-value"
...
def test_refresh_token_is_persisted() -> None:
    ...
    assert core.persist_accounts([row])[0]["credentials"]["refresh_token"] == "rt-value"
...
def test_row_from_account_loads_refresh_token() -> None:
        ...
        schema.parse_account(
            {
                ...
                "credentials": {"access_token": "t", "refresh_token": "rt"},
                "tasks": [{"id": "daily"}],
            }
        )
    )
