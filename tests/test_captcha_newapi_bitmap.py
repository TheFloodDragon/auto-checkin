...
# ── 浏览器脚本 helper 接线 ───────────────────────────────────────────────────
def _solve(image, scheme: str = "image:newapi_bitmap") -> str:
    """走统一解算注册表跑一次识别（纯计算，不碰浏览器）。"""
    import asyncio
    ...
    result = asyncio.run(SOLVERS.solve(_Ctx(), scheme, image=image))
...
def test_helper_solves_data_url() -> None:
    ...
    assert _solve(f"data:image/png;base64,{b64}") == label


def test_helper_solves_raw_bytes() -> None:
    label, b64 = REAL_SAMPLES[1]
    assert _solve(base64.b64decode(b64)) == label
...
def test_unknown_solver_id_fails_loudly() -> None:
    """未知解算方案必须明确报错，而不是静默返回空串。

    静默返回空会被调用方当成「这张图没认出来」，于是白白消耗掉几轮重试，真正的
    原因（方案名写错了）却一次都不会出现在日志里。
    """
    import asyncio
    ...
    with pytest.raises(ConfigError):
        asyncio.run(SOLVERS.solve(_Ctx(), "nope", image=b""))
