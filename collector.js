 ...
 *
 * 输出直接是 v3 账号对象（template / login / tasks / credentials），
...
(async () => {
  ...
  if (siteType === 'sub2api') {
    ...
    if (checkinEndpoint) {
      ...
      checkinAction = 'api';
      warnings.push('⚠️  未探测到签到接口；若该站页面上有「签到」按钮，请把 template 指向一个站点模板（scripts/tasks/*.py），tasks[].method 用 "script"');
    }
    ...
    if (hasCheckin) {
      ...
      warnings.push('   已建议 tasks[].method="relogin"：用浏览器重放 OAuth 登录来真正触发发放。');
      warnings.push('   需先在管理界面「捕获 OAuth 登录态」；若只想保活+监控余额，可改为 "visit"。');
    } else {
      checkinAction = 'visit';
      warnings.push('💡 未探测到签到接口，已建议 tasks[].method="visit"（保活 + 余额监控，不触发发放）。');
    }
  ...
  // ── 组装输出（v3 账号对象）────────────────────────────────────────────────
  // 直接产出配置文件里的形态：模板 + 登录方式 + 任务列表 + 凭据分组。
  // 探测到的三件事分别落到三处——站点族→template，认证方式→login.method，
  // 触发方式→tasks[0].method，不再是过去那种平铺的三维字段。
  const LOGIN_METHOD = { access_token: 'access_token', cookie: 'cookie', oauth: 'oauth' };
  const TASK_METHOD = { api: 'http_api', visit: 'visit', relogin: 'relogin', browser_script: 'script' };

  const slug = (baseUrl.replace(/^https?:\/\//, '').split(/[/:]/)[0] || 'site')
    .toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');

  const login = { method: LOGIN_METHOD[authMethod] || 'access_token' };
  if (userId) login.args = { user_id: userId };
  if (checkinAction === 'relogin') {
    login.method = 'oauth';
    login.provider = 'linuxdo';
  }

  const credentials = {};
  if (accessToken) credentials.access_token = accessToken;
  if (refreshToken) credentials.refresh_token = refreshToken;
  if (login.method === 'cookie' && visibleCookie) credentials.cookie = visibleCookie;
  ...
  const entry = {
    ...
    enabled: true,
    login,
    tasks: [{ id: 'daily', method: TASK_METHOD[checkinAction] || 'http_api' }],
  };
  if (Object.keys(credentials).length) entry.credentials = credentials;
  ...
  console.log(`  模板       : ${siteType}`);
  console.log(`  站点名称   : ${siteName}`);
  console.log(`  站点地址   : ${baseUrl}`);
  console.log(`  登录方式   : ${entry.login.method}`);
  console.log(`  任务方式   : ${entry.tasks[0].method}${checkinEndpoint ? `（${checkinEndpoint}）` : ''}`);
  ...
  console.log('  · template=站点长什么样，login.method=怎么认证，tasks[].method=怎么触发');
  console.log('  · 也可以直接在管理界面（python manage.py）里「从剪贴板导入」');
