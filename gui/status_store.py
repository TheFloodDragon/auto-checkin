...
from core.account import normalize_base_url
...
def _detail_quota_usd(detail: Any) -> float | None:
    from .core import detail_quota_usd
...
class StatusStore:
    ...
    def __init__(self, results_dir: Path | None = None, *, autosave: bool = True):
        self.results_dir = results_dir or _paths.RESULTS_DIR
    ...
    def site_group_key(row: SiteRow) -> str:
        ...
        return normalize_base_url(row.base_url) or (row.name or "").strip()
    ...
    def _load_batch_results(self) -> None:
        ...
        for item in rows:
            if not isinstance(item, dict):
                continue
            base = normalize_base_url(str(item.get("base_url") or ""))
            # v2 结果用 name，v1 用 site；两种文件在过渡期会同时存在。
            name = str(item.get("name") or item.get("site") or "")
            if not base and not name:
                continue
            quota_usd = None
            # v2 把可展示数值统一放在 text（自定义文本），v1 是 current_quota。
            raw_text = str(item.get("text") or item.get("current_quota") or "").lstrip("$")
            try:
                quota_usd = float(raw_text) if raw_text else None
            except ValueError:
                quota_usd = None
            status = str(item.get("legacy_status") or item.get("status") or "")
            ok = bool(item.get("ok")) if "ok" in item else status in ("success", "already_done")
        ...
        with _paths.file_lock(path):
            ...
            for key, entry in candidate_entries.items():
                ...
                if existing is None or _is_newer(entry.get("saved_at"), existing.get("saved_at")):
                    merged[str(key)] = entry
            _paths.atomic_write_text(path, json.dumps({"entries": merged}, ensure_ascii=False, indent=2))
