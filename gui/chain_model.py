...
class ChainDocument:
    ...
    def positions(self) -> dict[str, list[float]]:
        ...
        for key, point in raw.items():
            if isinstance(point, (list, tuple)) and len(point) == 2 and all(
                type(v) in (int, float) and math.isfinite(v) and abs(v) <= 100000 for v in point
            ):
    ...
    def _explicit(self) -> None:
        self._editable()
        links = dict(self.links())
        self.raw.setdefault("entry", self.entry())
        for item in self.raw["steps"]:
    ...
    def add(self, kind: str, *, after: str = "", position: list[float] | None = None) -> str:
        ...
        if after:
            parent = next((item for item in self.raw["steps"] if item["id"] == after), None)
            if parent is None:
                raise ConfigError("插入位置已不存在")
            node["on_failure"] = parent["on_failure"]
            parent["on_failure"] = key
        self.raw["steps"].append(node)
        if len(self.raw["steps"]) == 1:
            self.raw["entry"] = key
    ...
    def update(self, key: str, patch: dict, remove: tuple[str, ...] = ()) -> None:
        self._editable()
        item = next((item for item in self.raw["steps"] if item.get("id") == key), None)
        if item is None:
            raise ConfigError("步骤已不存在")
    ...
    def connect(self, source: str, target: str) -> None:
        ...
        backup = self.payload()
        self._explicit()
        next(item for item in self.raw["steps"] if item["id"] == source)["on_failure"] = target
        try:
    ...
    def delete(self, keys: list[str]) -> None:
        self._explicit()
        doomed = set(keys)
        self.raw["steps"] = [step for step in self.raw["steps"] if step["id"] not in doomed]
        for item in self.raw["steps"]:
    ...
    def set_positions(self, values: dict[str, list[float]]) -> None:
        ...
        if not isinstance(current, dict):
            current = {}
        current.update({key: [round(float(v), 1) for v in point] for key, point in values.items()})
