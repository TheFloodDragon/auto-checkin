...
两条约束不变：
- **加载先在临时对象里完成**，任何异常都不污染当前内存状态；
- **保存前在主线程冻结快照**，后台线程不再读可变的表单模型。
"""
...
def load_configuration() -> LoadedConfiguration:
    ...
    rows = [core.row_from_account(spec) for spec in document.accounts]
    oauth_states = {
        provider: {"accounts": dict(entry.get("accounts") or {})}
        for provider, entry in document.oauth_states.items()
    }
    for note in document.notes:
        core.bg_log("INFO", f"[migrate] {note}")
...
class SaveRequest:
    ...
    def persist(self) -> int:
        """原子保存账号文档，再标记与旧基线相比发生变化的运行期凭据。"""
        _store.save(core.persist_document(self.rows, self.oauth_states))
