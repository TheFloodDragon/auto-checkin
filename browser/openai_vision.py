...
from net import http as _net_http
...
class OpenAIVisionClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        self._transport = transport or _net_http.request_url
    ...
    async def _request(self, payload: dict[str, Any]) -> Any:
        ...
        for attempt in range(attempts):
            try:
                ...
                # 底层 HTTP 层会把连接中断、读取超时等**无 HTTP 状态**的网络错误标成
                # 瞬时错误。只看 status 会让 "Remote end closed connection without
                # response"（status=None）单次即放弃；而解析/schema 异常不带瞬时标记，
                # 仍然不会被误重试。
                retryable = status in self._TRANSIENT_STATUSES or _is_transient(exc)
...
def _is_transient(exc: BaseException) -> bool:
    ...
    同时认两种形态：新的 ``TransientError``（reason=network_error），以及任何带
    ``transient=True`` 属性的旧式异常——测试与第三方 transport 都可能传后者。
    """
    if bool(getattr(exc, "transient", False)):
        return True
    return str(getattr(exc, "reason", "")) == "network_error"
